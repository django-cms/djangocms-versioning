...
from djangocms_versioning.constants import PUBLISHED, VERSION_STATES, DRAFT, UNPUBLISHED, ARCHIVED
...
if hasattr(cms_admin, "TreePublishRow") and hasattr(cms_admin, "TreePublishRowMenu"):
    ...
    def _get_indicator_legend(self, context, page, language):
        return (
            ("cms-pagetree-node-state cms-pagetree-node-state-published", _("Published")),
            ("cms-pagetree-node-state cms-pagetree-node-state-dirty", _("Changed")),
            ("cms-pagetree-node-state cms-pagetree-node-state-unpublished", _("Unpublished")),
    ...
    def get_indicator_status(page_content):
        ...
        if page_content:
            if not hasattr(page_content, "_indicator_status"):
                ...
                signature = {state: versions.filter(state=state).exists() for state, name in VERSION_STATES}
                if signature[DRAFT] and not signature[PUBLISHED]:
                    page_content._indicator_status = DRAFT
                    page_content._version = versions.filter(state=DRAFT).order_by("-pk").first()
                    ...
                    page_content._version = versions.filter(state=UNPUBLISHED).order_by("-pk").first()
                elif not any(signature.values()):
                    page_content._indicator_status = ARCHIVED
                    page_content._version = versions.filter(state=ARCHIVED).order_by("-pk").first()
                else:
    ...
    def _get_indicator(self, context, page, language):
        ...
        if status == PUBLISHED:
            return "cms-pagetree-node-state cms-pagetree-node-state-published published", _("Published")
        elif status == "changed":
            return "cms-pagetree-node-state cms-pagetree-node-state-dirty dirty", _("Unpublished changes")
        elif status == UNPUBLISHED:
            return "cms-pagetree-node-state cms-pagetree-node-state-unpublished unpublished", _("Unpublished")
        elif status == DRAFT:
            return "cms-pagetree-node-state cms-pagetree-node-state-unpublished unpublished", _("Draft")
        else:
            return "cms-pagetree-node-state cms-pagetree-node-state-empty empty", _("Empty")
            ...
            if context["request"].user.has_perm(
                ...
                if status == DRAFT or status == "changed":
                    menu.append((
                        ...
                        reverse("admin:djangocms_versioning_pagecontentversion_publish", args=(version.pk,)),
                         "js-cms-tree-lang-trigger",  # Triggers POST from the frontend
                    ))
                if status == UNPUBLISHED:
                    menu.append((
                        _("Revert from unpublish"), "cms-icon-check-o",
                        reverse("admin:djangocms_versioning_pagecontentversion_revert", args=(version.pk,)),
                         "js-cms-tree-lang-trigger",  # Triggers POST from the frontend
                    ))
                if status == PUBLISHED or status == "changed":
                    menu.append((
                        _("Unpublish"), "cms-icon-forbidden",
                        reverse("admin:djangocms_versioning_pagecontentversion_unpublish", args=(version.pk,)),
